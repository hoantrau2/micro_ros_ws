...
std_msgs__msg__Int32 msg; //used to store the data of the message.

void timer_callback(rcl_timer_t *timer, int64_t last_call_time) //last_call_time: the last time this callback was invoked.
{
    rcl_ret_t ret = rcl_publish(&publisher, &msg, NULL); //The rcl_publish function is used to publish a message via the publisher.
...
// ROSIDL_GET_MSG_TYPE_SUPPORT(std_msgs, msg, Int32): Retrieves information about the message type the publisher will publish. In this case, the message type is std_msgs::msg::Int32.
